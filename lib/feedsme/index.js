 ...
 * @param {String} apiVersion The build api version to use
 * @param {Fucntion} done Completion callback.
 * @api private
 */
Feedsme.prototype.change = async function change(env, { data, promote = true }, apiVersion = 'v1') {
  ...
  // exists
  //
  await this.trigger(env, data, promote, apiVersion);
 ...
 * @param {String} [apiVersion='v1'] Which build api to call
 * @returns {Promise} to be resolved
 */
Feedsme.prototype.trigger = function trigger(env, data, promote, apiVersion = 'v1') {
  const publish = env === 'dev' && data.__published;
  const pkg = this.extractLatest(data);

  return Promise.all([
    // Root or Main package triggers dependent builds
    this.triggerDep({ publish, promote, env, pkg, apiVersion }),
 ...
 * @api private
 */
Feedsme.prototype.triggerDep = async function triggerDep({ publish, promote, env, pkg, apiVersion = 'v1' }) {
  ...
  return Promise.all(
    dep.dependents.map(name => {
      return limiter(() => this._triggerOneDep({ publish, promote, rootPkg: pkg, env, name, releaseLine, apiVersion }));
    })
  );
};
 ...
 * @param {String} [options.apiVersion='v1'] Which carpenter build api to call
 * @returns {Promise} to be resolved
 * @api private
 */
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, promote, env, rootPkg, name, releaseLine, apiVersion = 'v1' }) {
 ...
 * @param {Boolean} opts.promote - Should the build be promoted?
 * @param {String} [opts.apiVersion="v1"] - Which carpenter build version to call
 * @returns {Promise} to be resolved
 */
Feedsme.prototype.build = function build({ data, promote, apiVersion = 'v1' }) {
  var self = this;
  ...
  return new Promise((resolve, reject) => {
    ...
    data = apiVersion === 'v1' ? data : { promote, data };
    //
    // Send build to carpenter and trace the build logs we receive back
    //
    this.carpenter[apiVersion].build({ data }, (err, buildLog) => {
      if (err) {
        return onError(err);
      }
      //
