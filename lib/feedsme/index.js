...
Feedsme.prototype.change = async function change(env, data) {
  ...
  await Promise.all([
    ...
    this.triggerDep({ publish, env, releaseLine, pkg, dep }),
 ...
 * @api private
 */
Feedsme.prototype.triggerDep = async function triggerDep({ publish, env, dep, releaseLine, pkg }) {
  const { name, version } = pkg;
  const spec = {
 ...
 * @api private
 */
Feedsme.prototype._triggerStrategy = function triggerReady({ releaseLine, rootPkg, pkg, env }) {
