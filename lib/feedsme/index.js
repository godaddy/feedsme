 ...
 * @api private
 */
Feedsme.prototype.change = async function change(env, { data, promote = true, config }) {
  ...
  //
  await this.resolve(env, pkg, config);
...
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, promote, env, rootPkg, name, releaseLine }) {
  ...
  //
  const { strategy, trigger } = this._triggerStrategy({ releaseVersion, releaseLine, env, rootPkg, pkg });
 ...
 * @api private
 */
Feedsme.prototype._triggerStrategy = function _triggerStrategy({ releaseLine, rootPkg, pkg, env }) {
 ...
 * @api private
 */
Feedsme.prototype.resolve = async function resolve(env, data, config) {
  this.log.debug(`Resolving dependent for ${ data.name }`);

  const { add: results, remove } = await this._getDependencies(data, config);
...
Feedsme.prototype._getDependencies = async function _getDependencies(data, config) {
  const { Package, DependentOf } = this.models;
  ...
  const { name } = data;
  let all;
  if (config && 'parentPackage' in config) {
    // TODO: parentPackage needs added to the config, feedsme client needs updated to send the config,
    // carpeenterd needs updated with the new feedsme api client and to send the new config
    const { parentPackage } = config;
    // No need to filter explicit dependencies, we should just accept them
    all = Object.keys(parentPackage);
  } else {
    const { dependencies } = data;

    // TODO: get last dependency after filtering
    // Filter out dependencies not managed by warehouse
    all = await pFilter(Object.keys(dependencies), filterer, opts);
  }

  // TODO: we should get all the dependencies we already know about and delta them upfront so we can remove dependencies that no longer exist
  const depOf = await DependentOf.get(name);
  const remove = depOf.filter()

  // TODO: we should only ever be adding a single parent dependency, we need to not allow multiple anymore since it doesn't quite work as-is anyway.
  return {
    add: all,
    remove
  }
