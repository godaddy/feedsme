 ...
 * @param {Object} options.depOf Parent package lookup if it exists
 ...
 * @api private
 */
Feedsme.prototype.triggerDep = async function triggerDep({ publish, env, pkg }) {
  ...
  return Promise.all(
    dep.dependents.map(name => {
      return limiter(() => this._triggerOneDep({ publish, rootPkg: pkg, env, name, releaseLine }));
    })
  );
};
 ...
 * @api private
 */
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, env, rootPkg, name, releaseLine }) {
...
Feedsme.prototype.increment = function inc({ publish, strategy, payload, head, previousReleaseVersion, releaseVersion }) {
  ...
  this.log.info(`Potentially incrementing ${name} from (${prevVersion}|${releaseVersion}|${previousReleaseVersion}) to ${version} using ${strategy}.`);
  this.log.info(`Previous latest dist-tag for ${name} is ${payload && payload['dist-tags'] && payload['dist-tags'].latest}.`);
  this.log.debug(`Previous versions for ${name}`, { versions: payload && payload.versions });
 ...
 * @returns {Promise} to be resolved
 */
Feedsme.prototype.build = function build(data) {
  var self = this;
