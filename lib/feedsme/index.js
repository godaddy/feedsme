 ...
 * @api private
 */
Feedsme.prototype.increment = function inc(data, head) {
  const latest = clone(this.extractLatest(data));
  const name = latest.name || data.name;
  const prevVersion = latest.version || data.version;
  const prevTar = name + '-' + prevVersion + '.tgz'
  //
  ...
  if (!latest.version || semver.lt(latest.version, head.version)) {
    version = semver.inc(head.version, 'prerelease');
    action = 'publish';
  }

  // Protect regular publishes that dont need the rest of this
  if (version === latest.version) return data;

  latest.version = version;
  data.version = version;
  data.versions = data.versions || {};
  data.versions[version] = latest;
  ...
  data._attachments[tar] = data._attachments[prevTar] || {};
  delete data._attachments[prevTar];
