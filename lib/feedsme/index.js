...
Feedsme.prototype.triggerDep = async function triggerDep({ publish, promote, env, pkg }) {
  ...
  const [releaseLine, dep] = await Promise.all([
    publish ? this.release.get({ pkg: name }) : this.release.get({ pkg: name, version }), // TODO
    Dependent.get({ name })
  ]);
  ...
  return Promise.all(
    dep.dependents.map(name => {
      return limiter(() => this._triggerOneDep({ publish, promote, rootPkg: pkg, env, name, locale, releaseLine }));
    })
  );
};
 ...
 * @api private
 */
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, promote, env, rootPkg, name, locale, releaseLine }) {
  ...
  const [pkg, registryRootPkg] = await Promise.all([
    Package.get({ name }),
  ...
  let [version, head] = await Promise.all([
    // Lets fetch the realeaseVersion when we are not a publish and it exists
    Version.get({ name: pkg.name, version: fetchVersion }),
    BuildHead.get({ env, name, locale })
  ]);

  head = head || { version: '0.0.0' };

  if (!version) return this.logs.debug(`Unable to find ${ pkgName(pkg) }`);
  //
  // Fetch attachment and setup the data structure properly
  //
  const data = await this._expandVersion({ Version, version, pkg, env });
 ...
 * @api private
 */
Feedsme.prototype._expandVersion = async function ({ Version, version, pkg, env }) {
  const npm = this.config.get('npm');

  this.log.info(`Getting attachments for ${ pkg.name }`);
  const body = await Version.getAttachment({ pkg: version, read: npm });
  if (!body) return null;
...
Feedsme.prototype._resolveDep = async function _resolveDep({ name, dependency }) {
  ...
  const dependent = await Dependent.get({ name: dependency });
