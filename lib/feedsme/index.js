...
Feedsme.prototype.triggerDep = async function triggerDep({ publish, env, pkg }) {
  ...
  const spec = {
    previousVersion: releaseLine && releaseLine.previousVersion,
    pkg: name,
    version
  };
...
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, env, rootPkg, name, releaseLine }) {
  ...
  //
  const { strategy, trigger } = this._triggerStrategy({ releaseLine, env, rootPkg, pkg });
  ...
  const [version, head = { version: '0.0.0' }] = await Promise.all([
    ...
    !publish && releaseVersion ? Version.get(`${pkg.name}@${releaseVersion}`) : Version.get(`${name}@${pkg.version}`),
 ...
 *
 * @param {Object} options Options to be used
 * @param {Function} condition Used to evaluate terminating condition
