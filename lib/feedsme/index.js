...
Feedsme.prototype._triggerOneDep = async function _triggerOneDep({ publish, promote, env, rootPkg, name, releaseLine }) {
  ...
  let fetchVersion;
  console.log('previousReleaseVersion', previousReleaseVersion);
...
Feedsme.prototype._triggerStrategy = function _triggerStrategy({ registryRootPkg, releaseLine, rootPkg, pkg, env }) {
  console.log('_triggerStrategy', { releaseLine, rootPkg, pkg, env });
  ...
  const rootPkgVersions = Object.keys(registryRootPkg.versions || {})
    .sort((y, x) => semver.compare(x, y));
  const latestRootPkgVersion = rootPkgVersions[0];
  ...
  console.log('inclusive', { inclusive, rootVersion, rootRange });
  ...
  console.log({ previousMajor, previousPublishedMajor, fetchReleaseVersion, trigger, strategy });
...
Feedsme.prototype._fetchFromRegistry = function (name) {
  return new Promise((resolve, reject) => {
    ...
    this.registry.get(uri, this.params,  (err, pkg) => {
